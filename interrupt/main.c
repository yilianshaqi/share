...
#define   ICCEOIR_CPU			(*(volatile unsigned int *) 0x10480010)
...
void interrupt(void)
{
		...
		ICDICPR_CPU = ICDICPR_CPU | (1<<(57-32))  ;   //中断信号清除
		EXT_INT41_PEND = EXT_INT41_PEND | (1<<1)  ;   //硬件中断发生寄存器清除
		ICCEOIR_CPU =  ICCEOIR_CPU  &~(0x3ff) | num;
}
